...
BOOL CDECL wine_get_steamclient_path( HMODULE module, LPWSTR dest,
                                      DWORD size, ULONG *result_len )
{
    BOOL is_steamclient = (steamclient_hmod != NULL &&
    ...
    if (is_steamclient)
    {
        ...
        if (ARRAY_SIZE(steamclient_pathW) > size)
        {
        ...
        {
            err = ERROR_SUCCESS;
            len = ARRAY_SIZE(steamclient_pathW) - 1;
        }

        if (len > 0)
        {
            memcpy( dest, steamclient_pathW, len * sizeof(WCHAR) );
        }
        if (len < size)
        {
            dest[len] = 0;
    ...
    return is_steamclient;
}

HMODULE CDECL wine_get_steamclient_if_substr( LPCWSTR str )
{
    return (steamclient_hmod && strcasestrW( str, steamclientW ))
           ? steamclient_hmod
           : NULL;
}
